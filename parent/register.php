...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['link_student'])) {
    if (empty($_SESSION['parent_reg_id'])) {
        ...
        if (empty($regNo)) {
            ...
            $student = mysqli_query($conn, "SELECT id, first_name, second_name, last_name, form_level, stream, registration_no FROM students WHERE registration_no='$regNo' AND is_active=1 LIMIT 1");
