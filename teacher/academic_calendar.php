...
.cal-wrap{background:#fff;border:1px solid #e5e7eb;border-radius:14px;padding:16px;box-shadow:0 2px 8px rgba(0,0,0,.03);}
#calendar{min-height:450px;}
.side-panel .panel{background:#fff;border:1px solid #e5e7eb;border-radius:14px;padding:14px;margin-bottom:12px;}
.side-panel .panel-title{font-size:12px;font-weight:700;color:#6b7280;text-transform:uppercase;letter-spacing:.5px;margin-bottom:8px;display:flex;align-items:center;gap:6px;}
...
.event-date{font-size:10px;color:#9ca3af;}
...
<div class="row g-3">
    <div class="col-md-3 side-panel">
        ...
        <div class="panel">
            ...
            <div style="font-size:12px;color:#9ca3af;text-align:center;padding:10px;">No ongoing events.</div>
            <?php else: ?>
        ...
        <div class="panel">
            ...
            <div style="font-size:12px;color:#9ca3af;text-align:center;padding:10px;">No upcoming events.</div>
            <?php else: ?>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    var calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        eventClick: function(info) {
            ...
            if (props.type) html += '<div style="font-size:12px;color:#6b7280;margin-bottom:4px;"><span style="font-weight:600;">Type:</span> ' + props.type + '</div>';
            var start = info.event.start ? info.event.start.toLocaleDateString() : '';
            var end = info.event.end ? info.event.end.toLocaleDateString() : '';
            if (start) html += '<div style="font-size:12px;color:#6b7280;margin-bottom:4px;"><span style="font-weight:600;">Start:</span> ' + start + '</div>';
            if (end && end !== start) html += '<div style="font-size:12px;color:#6b7280;margin-bottom:4px;"><span style="font-weight:600;">End:</span> ' + end + '</div>';
