...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create' || $action === 'update') {
        ...
        if ($doc_type === 'file' && isset($_FILES['file']) && $_FILES['file']['error'] === UPLOAD_ERR_OK) {
            ...
            move_uploaded_file($_FILES['file']['tmp_name'], "../uploads/syllabus/$file_path");
        }
        if ($action === 'create') {
            $fp = $file_path ? "'$file_path'" : 'NULL';
            $lu = $link_url ? "'$link_url'" : 'NULL';
            $fl = $form_level ? "'$form_level'" : 'NULL';
            mysqli_query($conn, "INSERT INTO subject_syllabus (subject_id, title, description, file_path, link_url, doc_type, form_level, created_by) VALUES ($subject_id, '$title', '$desc', $fp, $lu, '$doc_type', $fl, $_SESSION[admin_id])");
            $_SESSION['flash'] = ['msg' => 'Syllabus added.', 'type' => 'success'];
        } else {
            $id = intval($_POST['id']);
            $fl = $form_level ? "'$form_level'" : 'NULL';
            if ($file_path) {
                mysqli_query($conn, "UPDATE subject_syllabus SET subject_id=$subject_id, title='$title', description='$desc', file_path='$file_path', link_url='', doc_type='$doc_type', form_level=$fl WHERE id=$id");
            } else {
                mysqli_query($conn, "UPDATE subject_syllabus SET subject_id=$subject_id, title='$title', description='$desc', link_url='$link_url', doc_type='$doc_type', form_level=$fl WHERE id=$id");
            }
            $_SESSION['flash'] = ['msg' => 'Syllabus updated.', 'type' => 'success'];
        }
    }
    if ($action === 'delete') {
        $id = intval($_POST['id']);
        $r = mysqli_fetch_assoc(mysqli_query($conn, "SELECT file_path FROM subject_syllabus WHERE id=$id"));
        if ($r && $r['file_path'] && file_exists("../uploads/syllabus/".$r['file_path'])) unlink("../uploads/syllabus/".$r['file_path']);
        mysqli_query($conn, "DELETE FROM subject_syllabus WHERE id=$id");
        $_SESSION['flash'] = ['msg' => 'Syllabus deleted.', 'type' => 'success'];
    }
    header("Location: manage_syllabus.php"); exit();
}

$docs = mysqli_query($conn, "SELECT ss.*, sub.subject_name FROM subject_syllabus ss JOIN subjects sub ON sub.id = ss.subject_id ORDER BY sub.subject_name, ss.created_at DESC");
...
<div class="page-title"><i class="bi bi-journal-text"></i> Subject Syllabus</div>
...
<div class="card">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;">
        <div style="font-size:14px;font-weight:700;color:#0b2b3f;">Syllabus by Subject</div>
        <button class="btn-sm btn-primary-sm" data-bs-toggle="modal" data-bs-target="#createModal"><i class="bi bi-plus-lg"></i> Add Syllabus</button>
    </div>

    <?php if (mysqli_num_rows($docs) == 0): ?>
    <div class="empty"><i class="bi bi-journal-x" style="font-size:2rem;display:block;margin-bottom:8px;opacity:.3;"></i>No syllabus documents yet.</div>
    <?php else: ?>
    <table class="table">
        <thead><tr><th>Subject</th><th>Title</th><th>Form</th><th>Document</th><th style="text-align:right;">Actions</th></tr></thead>
        <tbody>
            <?php while ($d = mysqli_fetch_assoc($docs)): ?>
            <tr>
                <td><span class="subj-tag"><?= htmlspecialchars($d['subject_name']) ?></span></td>
                <td><strong><?= htmlspecialchars($d['title']) ?></strong></td>
                <td><?= htmlspecialchars($d['form_level'] ?? 'All') ?></td>
                <td>
                    <?php if ($d['doc_type'] === 'file' && $d['file_path']): ?>
                    <a class="doc-link" href="../uploads/syllabus/<?= urlencode($d['file_path']) ?>" target="_blank"><i class="bi bi-file-earmark-pdf"></i> View</a>
                    <?php elseif ($d['doc_type'] === 'link' && $d['link_url']): ?>
                    <a class="doc-link" href="<?= htmlspecialchars($d['link_url']) ?>" target="_blank"><i class="bi bi-link-45deg"></i> Open</a>
                    <?php else: ?>
                ...
                <td style="text-align:right;">
                    ...
                    <button class="btn-sm btn-danger-sm" onclick="confirmDelete('Delete this syllabus?', <?= $d['id'] ?>)"><i class="bi bi-trash3"></i></button>
                </td>
            </tr>
            <?php endwhile; ?>
        </tbody>
        ...
        <div class="modal-content">
            <form method="post" enctype="multipart/form-data">
                <div class="modal-header border-0 pb-0">
                    <h5 class="modal-title"><i class="bi bi-plus-square"></i> Add Syllabus</h5>
                ...
                <div class="modal-body">
                    ...
                    <div class="f-group">
                        ...
                        <input class="f-input" name="title" required placeholder="e.g. Mathematics Syllabus 2026">
                    </div>
                    <div class="f-group">
...
<div class="modal fade" id="editModal<?= $d['id'] ?>" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <form method="post" enctype="multipart/form-data">
                <div class="modal-header border-0 pb-0">
                    <h5 class="modal-title"><i class="bi bi-pencil-square"></i> Edit Syllabus</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    ...
                    <div class="f-group" id="editLinkGroup<?= $d['id'] ?>" style="<?= $d['doc_type'] === 'file' ? 'display:none;' : '' ?>">
                        <label class="f-label">External URL</label>
                        <input class="f-input" type="url" name="link_url" value="<?= htmlspecialchars($d['link_url'] ?? '') ?>">
                    </div>
                </div>
                <div class="modal-footer border-0 pt-0">
                    <button type="button" class="btn btn-sm btn-secondary" data-bs-dismiss="modal">Cancel</button>
