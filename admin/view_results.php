...
$chkCountSql = "SELECT COUNT(*) total FROM exam_results_summary ers JOIN students s ON s.id=ers.student_id WHERE ers.exam_id='$exam_id' $flFilterSql";
...
if($summary_json && isset($summary_json['divisions'])){
    ...
    $divQ = mysqli_query($conn,"SELECT ers.division,s.sex,COUNT(*) as c FROM exam_results_summary ers JOIN students s ON s.id=ers.student_id WHERE ers.exam_id='$exam_id' $flFilterSql GROUP BY ers.division,s.sex");
    $divisions = [];
...
<table class="results-table">
    ...
    <tbody>
        ...
        <tr>
            ...
            <?php if(!$form_level_filter): ?><td data-label="Form"><?= str_replace('Form ','F.',htmlspecialchars($st['form_level']??'')) ?></td><?php endif; ?>
